...
AETHER._EXECUTOR = detectExecutor()

-- Safe wrappers for executor-specific functions
getgenv().safeGetgc = function()
    if getgc then
        local s, r = pcall(getgc)
        return s and r or {}
    end
    return {}
end

getgenv().safeGetnilinstances = function()
    if getnilinstances then
        local s, r = pcall(getnilinstances)
        return s and r or {}
    end
    return {}
end

getgenv().safeFireproximityprompt = function(prompt)
    if fireproximityprompt then
        local s = pcall(fireproximityprompt, prompt)
        return s
    end
...
getgenv().safeGetupvalues = function(func)
    if debug and debug.getupvalues then
        local s, r = pcall(debug.getupvalues, func)
        return s and r or {}
    end
    if getupvalues then
...
getgenv().safeSetupvalue = function(func, idx, val)
    if debug and debug.setupvalue then
        return pcall(debug.setupvalue, func, idx, val)
    end
    if setupvalue then
        return pcall(setupvalue, func, idx, val)
    end
    return false
end
...
-- Print executor info
print("═══════════════════════════════════════")
print("       AETHER HUB - Blox Fruits")
print("       Executor: " .. AETHER._EXECUTOR)
print("       Version: " .. AETHER._VERSION)
print("═══════════════════════════════════════")
...
elseif game.PlaceId == 7449423635 then
    ...
    game:GetService("Players").LocalPlayer:Kick("AETHER HUB: Game not supported. Please wait for updates...")
end
...
function TweenTempleLegit()
    ...
    local STOP, STOPRL = nil, nil
    
    pcall(function()
        -- Wait for CombatFramework to load
        if Client:FindFirstChild("PlayerScripts") and Client.PlayerScripts:FindFirstChild("CombatFramework") then
            STOP = require(Client.PlayerScripts.CombatFramework.Particle)
...
function GetBladeHit()
    local result = nil
    pcall(function()
        if game:GetService("Players").LocalPlayer:FindFirstChild("PlayerScripts") and 
           game:GetService("Players").LocalPlayer.PlayerScripts:FindFirstChild("CombatFramework") then
...
function AttackHit()
    pcall(function()
        if not game:GetService("Players").LocalPlayer:FindFirstChild("PlayerScripts") or
           not game:GetService("Players").LocalPlayer.PlayerScripts:FindFirstChild("CombatFramework") then
            return
        end
        local CombatFrameworkLib = debug.getupvalues(require(game:GetService("Players").LocalPlayer.PlayerScripts.CombatFramework))
        local CmrFwLib = CombatFrameworkLib[2]
